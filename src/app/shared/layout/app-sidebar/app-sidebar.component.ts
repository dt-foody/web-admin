...
export class AppSidebarComponent implements OnInit, OnDestroy {
  ...
  navItems: NavItem[] = [
    ...
    },
    {
      ...
      subItems: [
        ...
        { name: 'Đối soát', path: '/billing' }, // hoặc 'Công nợ' tùy business
      ],
    },
    {
  ...
  isActive(path: string): boolean {
    ...
    // Nếu path là "/", chỉ active đúng chính xác "/"
    if (path === '/') {
      return currentUrl === '/';
    }

    // Ngược lại, active nếu currentUrl bắt đầu bằng path
  ...
  isParentActive(nav: NavItem): boolean {
    ...
    return nav.subItems.some((sub) => currentUrl.startsWith(sub.path));
  }
      ...
      group.items.forEach((nav, i) => {
        if (nav.subItems) {
          nav.subItems.forEach((subItem) => {
            if (currentUrl.startsWith(subItem.path)) {
